...
class InfoManager(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Крок 8: Статистика використання
        self.add_step(
            scroll_frame,
            "8. Статистика використання",
            "Біля кожної програми у списку 'Програми' тепер видно позначку '▶ N' — скільки разів "
            "цю програму реально було запущено через лаунчер (вручну, з набору, за розкладом чи в "
            "автозапуску). Це допомагає зрозуміти, який софт дійсно потрібен під рукою, а який давно "
            "не використовується і його можна прибрати зі списку.\n\n"
            "Повний графік по всіх програмах доступний у вкладці 'Налаштування' — кнопка "
            "'📊 Переглянути статистику...' відкриває окреме вікно з наочним графіком використання "
            "та кнопкою '🗑 Скинути всю статистику', якщо захочеться почати відлік заново.\n\n"
            "Скинути лічильник окремої програми можна і без цього вікна — правий клік по програмі → "
            "'🔄 Скинути лічильник запусків'."
        )
        ...
        version_label = ctk.CTkLabel(footer_frame, text="Версія: 1.9.0 (Stats Update)", font=(None, 11),
                                     text_color="gray")
    ...
    def add_customization_step(self, master):
        ...
        t_lbl = ctk.CTkLabel(box, text="9. Кастомізація інтерфейсу та готові теми", font=(None, 13, "bold"),
