...
def create_tray_image():
    """ Іконка для системного трею. Раніше тут просто робився
    Image.open(...) на launcher.ico — Pillow за замовчуванням віддає
    НАЙБІЛЬШИЙ вшитий кадр (256x256), і вже ЙОГО pystray стискав "на
    льоту" до крихітного розміру трею (зазвичай 16x16 чи трохи більше
    при масштабуванні екрану) — це зменшення в 16+ разів, і навіть
    непоганий алгоритм стиснення на такому перепаді дає помітну
    розмитість, причому вона відбувається щоразу заново при кожному
    запуску, вже ПІСЛЯ всіх наших покращень усередині самого .ico.

    Тепер натомість явно береться вже готовий кадр 64x64, вшитий у
    .ico (той самий, який вручну підрізкований Unsharp Mask) — pystray
    все одно може трохи зменшити його під конкретний розмір трею, але
    це вже набагато менший перепад (64→16, а не 256→16), тож підсумкова
    іконка в треї виходить помітно чіткішою. """
    try:
        icon_file = resource_path(os.path.join("assets", "launcher.ico"))
